                        ...
                        <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
                            ...
                            <li class="flex">
                                <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                                    ...
                                    <i class="fa-solid fa-plus w-4 h-4 mr-3"></i>
                                    <span>Assignn cow to staff</span>
                                </a>
                            </li>
                            <li class="flex">
                                <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
            ...
            <li class="relative">
                <button class="align-middle rounded-full focus:shadow-outline-purple focus:outline-none" @click="toggleProfileMenu"
                    ...
                    @if (auth()->user()->user_type == 1)
                        <img src="{{ asset('admin/assets/img/admin.png') }}" class="object-cover bg-white w-8 h-8 rounded-full" alt=""
                            aria-hidden="true">
                    @elseif (auth()->user()->user_type == 2)
                        <img src="{{ asset('admin/assets/img/manager.png') }}" class="object-cover bg-white w-8 h-8 rounded-full" alt=""
                            aria-hidden="true">
                    @elseif (auth()->user()->user_type == 3)
                        <img src="{{ asset('admin/assets/img/staff.png') }}" class="object-cover bg-white w-8 h-8 rounded-full" alt=""
                            aria-hidden="true">
                    @endif
                </button>
                <template x-if="isProfileMenuOpen">
                    <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                        ...
                        <li class="flex">
                            <span
                                class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md bg-gray-100 hover:text-gray-800 dark:bg-gray-800 dark:hover:text-gray-200 select-none">
                                <svg class="w-4 h-4 mr-3" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                                    <path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                </svg>
                                <span class="select-none">{{ auth()->user()->name }}</span>
                            </span>
                        </li>
                        <li class="flex">
                            <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                        ...
                        <li class="flex">
                            <form method="POST" action="{{ route('logout') }}"
                                ...
                                @csrf
                                <a class="inline-flex items-center text-red-600" href="{{ route('logout') }}"
