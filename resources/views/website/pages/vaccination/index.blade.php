                ...
                <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            ...
                                            <button
                                                ...
                                                aria-label="Delete" id="deleteButton{{ $vaccination->id }}" data-id="{{ $vaccination->id }}"
                                                onclick="dataDelete({{ $vaccination->id }})" @click="openModal">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"
                            ...
                            @empty
                                <tr colspan="4">
                                    <p class="text-center text-white-100 dark:text-gray-400"> No vaccination data found. </p>
                                </tr>
                            @endforelse
                    ...
                    {{ $vaccinations->onEachSide(5)->links() }}
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    <script>
        function dataDelete(id) {
            ...
            let route = "{{ route('vaccination.destroy', ['vaccination' => ':id']) }}";
