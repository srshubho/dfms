                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead>
                            <tr
                                ...
                                <th class="px-4 py-3">Breed Percentage</th>
                                <th class="px-4 py-3">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @forelse ($bulls as $bull)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            <a href="{{ route('bull.edit', $bull->id) }}">
                                            ...
                                            <button
                                                ...
                                                aria-label="Delete" id="deleteButton{{ $bull->id }}" data-id="{{ $bull->id }}"
                                                onclick="dataDelete({{ $bull->id }})" @click="openModal">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"
...
@push('scripts')
    <script>
        function dataDelete(id) {
            ...
            let route = "{{ route('bull.destroy', ['bull' => ':id']) }}";
