...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <form action="{{ route('insemination.update', $insemination->id) }}" method="POST">
                {{ csrf_field() }}
                @method('put')
                    ...
                    <select name="cow_id"
                        ...
                        @foreach ($cows as $cow)
                            <option value="{{ $cow->id }}" {{ old('cow_id') ? 'selected' : '' }}
                                {{ $insemination->cow_id == $cow->id ? 'selected' : '' }}>
                                {{ $cow->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="bull_id"
                        ...
                        @foreach ($bulls as $bull)
                            <option value="{{ $bull->id }}" {{ old('bull_id') ? 'selected' : '' }}
                                {{ $insemination->bull_id == $bull->id ? 'selected' : '' }}>
                                {{ $bull->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <label class="block mt-4 text-sm">
                    ...
                    <input type="date" name="insemination_date" id="insemination_date"
                        value="{{ old('insemination_date') ? old('insemination_date') : $insemination->insemination_date }}" required
